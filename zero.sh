...
echo "║                    Developer Info                    ║"
echo "╚══════════════════════════════════════════════════════╝"
echo -e "${WHITE}OWNER  : ${ORANGE}YudaMods"
echo -e "${WHITE}TELEGRAM: ${ORANGE}t.me/YUDAMODSZX"
echo -e "${RED}© 2026 YudaMods Dev${RESET}"
...
case "$OPTION" in
     1)
        ...
        REPO_URL="https://github.com/axelvers4-cmd/installer-premium.git"
        TEMP_DIR="installer-premium"

        git clone "https://${GITHUB_TOKEN}@github.com/axelvers4-cmd/installer-premium.git" "$TEMP_DIR"
     ...
     2)
        ...
        THEME_NAME="stellar"
        THEME_URL="https://github.com/axelvers4-cmd/installer-premium/raw/main/${THEME_NAME}.zip"
     ...
     5)
        ...
        FILE_URL="https://github.com/axelvers4-cmd/installer-premium/raw/main/Slate-v1.0.zip"
        ...
        FILE_URL="https://github.com/axelvers4-cmd/installer-premium/raw/main/autosuspens.zip"
    ...
    FILE_URL="https://raw.githubusercontent.com/axelvers4-cmd/installer-premium/main/addons/googleanalytic/config.json"
...
;;
     9)
# Default URL gambar
DEFAULT_URL="https://i.postimg.cc/8s5k52d4/fea8c40b4afe53238b459b8cb0369545.jpg"
...
if [ $? -eq 0 ]; then
    ...
    echo "GAGAL MENGGANTI FILE! Hubungi @YUDAMODSZX untuk bantuan."
fi
;;
...
GITHUB_TOKEN="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"
...
FILE_URL="https://raw.githubusercontent.com/axelvers4-cmd/installer-premium/main/addons/cookies/config.json"
...
in_func == 1 && /^\s*{/ {
    ...
    print "            throw new DisplayException(\"Akses ditolak. Penghapusan user hanya dapat dilakukan dengan persetujuan Administrator (ID 1)." version "\");";
    print "        }";
    in_func = 0; next;
}
{ print }
...
in_func == 1 && /^\s*{/ {
    ...
    print "            throw new DisplayException(\"Akses ditolak. Penghapusan server hanya dapat dilakukan dengan persetujuan Administrator (ID 1)." version "\");";
    print "        }";
    in_func = 0; next;
}
{ print }
...
20)
    ...
    for name in "${!CONTROLLERS[@]}"; do
        ...
        in_func == 1 && /^\s*{/ {
            ...
            print "        $user = Auth::user();";
            print "        if (!$user || $user->id !== " admin_id ") {";
            print "            abort(403, \"YudaMods - Akses ditolak\");";
            print "        }";
            in_func = 0; next;
        }
        { print; }
